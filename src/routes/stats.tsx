...
export const Route = createFileRoute("/stats")({
  head: () => ({ meta: [{ title: "Stats — F26 Tracker" }] }),
  component: StatsPage,
});
...
function StatsPage() {
      ...
      <AppLayout title="Statistics">
        <Card className="glass-card">
          <CardContent className="py-12 text-center text-muted-foreground">
            ...
            Play some matches to unlock stats.
          </CardContent>
        </Card>
      </AppLayout>
    );
  ...
  const usage = teamUsage(matches.data ?? [], teams.data ?? []).slice(0, 8)
    .map(t => ({ name: t.team.name, value: t.count }));

  return (
    <AppLayout title="Statistics" subtitle="Charts across players and teams">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Most active players" data={mostActive} />
        <ChartCard title="Most wins" data={mostWins} />
        <ChartCard title="Top scorers (goals)" data={topScorers} />
        <ChartCard title="Win rate (%)" data={winRate} suffix="%" />
        <div className="lg:col-span-2">
          <ChartCard title="Team usage frequency" data={usage} />
        </div>
      </div>
    </AppLayout>
  );
  ...
  return (
    <Card className="glass-card">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-display uppercase tracking-[0.2em] text-muted-foreground">{title}</CardTitle>
      </CardHeader>
      <CardContent>
        {data.length === 0 ? (
